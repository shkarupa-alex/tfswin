...
def window_partition(inputs, height, width, window_size, dtype=None, name=None):
    with backend.name_scope(name or 'window_partition'):
        ...
        outputs = ops.reshape(inputs,
                              [-1, windows_height, window_size, windows_width,
                               window_size, channels])
        outputs = ops.transpose(outputs, [0, 1, 3, 2, 4, 5])
        outputs = ops.reshape(outputs, [-1, window_size ** 2, channels])

        return outputs


def window_reverse(inputs, height, width, window_size, dtype=None, name=None):
    with backend.name_scope(name or 'window_reverse'):
        inputs = backend.convert_to_tensor(inputs, dtype)
        ...
        outputs = ops.reshape(inputs,
                              [-1, windows_height, windows_width, window_size,
                               window_size, channels])
        outputs = ops.transpose(outputs, [0, 1, 3, 2, 4, 5])
...
def window_partition_fused(inputs, height, width, window_size, num_heads,
                           qkv_mult=3, dtype=None, name=None):
    with backend.name_scope(name or 'window_partition'):
        inputs = backend.convert_to_tensor(inputs, dtype)
        ...
        outputs = ops.reshape(
            inputs,
            [-1, windows_height, window_size, windows_width, window_size,
             qkv_mult, num_heads, head_channels])
        outputs = ops.transpose(outputs, [5, 0, 1, 3, 6, 2, 4, 7])
        outputs = ops.reshape(outputs,
                              [qkv_mult, -1, num_heads, window_size ** 2,
                               head_channels])

        return outputs


def window_reverse_fused(inputs, height, width, window_size, num_heads,
    ...
    with backend.name_scope(name or 'window_reverse'):
        ...
        outputs = ops.reshape(
            inputs, [-1, windows_height, windows_width, num_heads, window_size,
                     window_size, head_channels])
